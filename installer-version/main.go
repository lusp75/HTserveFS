...
var staticFilesPath = "./assets"

// Installer configuration
type InstallerConfig struct {
	AssetsPath       string   `json:"assets_path"`
	FallbackEmbedded bool     `json:"fallback_embedded"`
	Security         struct {
...
// Extended config with installer settings
type ExtendedAppConfig struct {
...
// validateAssetFile validates external asset files for security
func validateAssetFile(filePath string) error {
	if !installerConfig.Security.ValidateAssets {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("asset file not found: %s", filePath)
	}

	// Check file size
	if info.Size() > 10*1024*1024 { // 10MB limit for assets
		return fmt.Errorf("asset file too large: %s", filePath)
	}

	// Check file extension
	ext := strings.ToLower(filepath.Ext(filePath))
	allowed := false
	for _, allowedExt := range installerConfig.Security.AllowedExtensions {
		if ext == allowedExt {
			allowed = true
			break
		}
	...
	if !allowed {
		return fmt.Errorf("asset file extension not allowed: %s", ext)
	}
...
func main() {
	...
	// Serve static files from external filesystem with validation
	// Serve assets directly (CSS, JS files)
	r.GET("/assets/*filepath", func(c *gin.Context) {
		filePath := filepath.Join(staticFilesPath, "assets", c.Param("filepath"))
		if err := validateAssetFile(filePath); err != nil {
			log.Printf("[ERROR] Asset validation failed for %s: %v", filePath, err)
	...
	r.GET("/favicon.svg", func(c *gin.Context) {
		filePath := filepath.Join(staticFilesPath, "favicon.svg")
		if err := validateAssetFile(filePath); err != nil {
			log.Printf("[ERROR] Asset validation failed for %s: %v", filePath, err)
			c.String(404, "Not found")
			return
	...
	r.GET("/htservefs-icon.svg", func(c *gin.Context) {
		filePath := filepath.Join(staticFilesPath, "htservefs-icon.svg")
		if err := validateAssetFile(filePath); err != nil {
			log.Printf("[ERROR] Asset validation failed for %s: %v", filePath, err)
			c.String(404, "Not found")
			return
	...
	r.GET("/htservefs-icon.ico", func(c *gin.Context) {
		filePath := filepath.Join(staticFilesPath, "htservefs-icon.ico")
		if err := validateAssetFile(filePath); err != nil {
			log.Printf("[ERROR] Asset validation failed for %s: %v", filePath, err)
			c.String(404, "Not found")
			return
...
func loadConfig() {
	...
	installerConfig = InstallerConfig{
		...
		Security: struct {
			...
			ValidateAssets:    true,
			AllowedExtensions: []string{".html", ".css", ".js", ".svg", ".ico"},
			MaxFileSize:       "10MB",
		},
	}
	...
	// Try to load installer configuration from config-installer.json
	log.Printf("[DEBUG] loadConfig: attempting to load config-installer.json")
	if data, err := os.ReadFile("config-installer.json"); err == nil {
		log.Printf("[DEBUG] loadConfig: config-installer.json found, parsing...")
		var extendedConfig ExtendedAppConfig
		if err := json.Unmarshal(data, &extendedConfig); err == nil {
			log.Printf("[DEBUG] loadConfig: config-installer.json parsed successfully")
			installerConfig = extendedConfig.Installer
			staticFilesPath = installerConfig.AssetsPath
			log.Printf("[DEBUG] loadConfig: assets path set to: %s", staticFilesPath)
		} else {
			log.Printf("[ERROR] loadConfig: installer config JSON parsing failed: %v, using defaults", err)
		}
	} else {
		log.Printf("[DEBUG] loadConfig: config-installer.json not found: %v, using defaults", err)
	}
}